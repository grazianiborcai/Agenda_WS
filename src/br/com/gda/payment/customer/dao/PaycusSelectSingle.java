	...
	private static class ResultParser implements DaoResultParser<PaycusInfo> {
		...
		@Override public List<PaycusInfo> parseResult(ResultSet stmtResult, long lastId) throws SQLException {
			...
			do {
				...
				dataInfo.codPayCustomer = stmtResult.getLong(PaycusDbTableColumn.COL_COD_PAYCUS);
				...
				stmtResult.getLong(PaycusDbTableColumn.COL_COD_USER_SNAPSHOT);
				if (stmtResult.wasNull() == NOT_NULL)
					dataInfo.codUserSnapshot = stmtResult.getLong(PaycusDbTableColumn.COL_COD_USER_SNAPSHOT);
				
				stmtResult.getLong(PaycusDbTableColumn.COL_COD_CUSTOMER);
				if (stmtResult.wasNull() == NOT_NULL)
					dataInfo.codCustomer = stmtResult.getLong(PaycusDbTableColumn.COL_COD_CUSTOMER);
				
				stmtResult.getLong(PaycusDbTableColumn.COL_COD_CUSTOMER_SNAPSHOT);
				if (stmtResult.wasNull() == NOT_NULL)
					dataInfo.codCustomerSnapshot = stmtResult.getLong(PaycusDbTableColumn.COL_COD_CUSTOMER_SNAPSHOT);
				
				stmtResult.getLong(PaycusDbTableColumn.COL_COD_PAY_PARTNER);
				if (stmtResult.wasNull() == NOT_NULL)
					dataInfo.codPayPartner = stmtResult.getLong(PaycusDbTableColumn.COL_COD_PAY_PARTNER);
