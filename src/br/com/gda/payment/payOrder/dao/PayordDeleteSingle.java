...
public final class PayordDeleteSingle implements DaoStmt<PayordInfo> {
	private DaoStmt<PayordInfo> stmtSql;
	private DaoStmtOption<PayordInfo> stmtOption;	
	
	
	public PayordDeleteSingle(Connection conn, PayordInfo recordInfo, String schemaName) {
	...
	@Override public DaoStmt<PayordInfo> getNewInstance() {
		return new PayordDeleteSingle(stmtOption.conn, stmtOption.recordInfo, stmtOption.schemaName);
	}
	
	
	
	private class ResultParser implements DaoResultParser<PayordInfo> {
		@Override public List<PayordInfo> parseResult(ResultSet stmtResult, long lastId) throws SQLException {
			...
			PayordInfo emptyInfo = new PayordInfo();
