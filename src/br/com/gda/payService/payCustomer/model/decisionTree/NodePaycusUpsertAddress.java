...
import br.com.gda.payService.payCustomer.model.action.StdPaycusUpsertAddress;
import br.com.gda.payService.payCustomer.model.checker.PaycusCheckHasAddress;

public final class NodePaycusUpsertAddress implements DeciTree<PaycusInfo> {
	private DeciTree<PaycusInfo> tree;
	
	
	public NodePaycusUpsertAddress(DeciTreeOption<PaycusInfo> option) {
	...
	private ModelChecker<PaycusInfo> buildDecisionChecker(DeciTreeOption<PaycusInfo> option) {
		...
		checker = new PaycusCheckHasAddress(checkerOption);
		queue.add(checker);	
	...
	private List<ActionStd<PaycusInfo>> buildActionsOnPassed(DeciTreeOption<PaycusInfo> option) {
		...
		actions.add(new StdPaycusUpsertAddress(option));		
		return actions;
	}
