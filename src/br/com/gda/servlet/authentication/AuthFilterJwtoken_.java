...
public final class AuthFilterJwtoken_ extends OncePerRequestFilter {
	...
	public AuthFilterJwtoken_(AuthenticationManager authenticationManager) {
	...
	public AuthFilterJwtoken_(AuthenticationManager authenticationManager, AuthenticationEntryPoint authenticationEntryPoint) {
		Assert.notNull(authenticationManager, "authenticationManager cannot be null");
	...
	private AuthToken customGetAuthenticationToken(String username, String password, HttpServletRequest request) {
		String tokenHeader = customGetJwtoken(request);
		JwtokenInfo jwtoken = customParseJwtoken(tokenHeader);
		...
		return new AuthToken(username, password, String.valueOf(jwtoken.codOwner), language, jwtoken.codPlatform);
	}
	
	
	
	private String customGetJwtoken(HttpServletRequest request) {
	...
	private JwtokenInfo customParseJwtoken(String jwtoken) {
		JwtokenInfo recordInfo = new JwtokenInfo();
		recordInfo.tokenToVerify = jwtoken;
		
		DeciTree<JwtokenInfo> parser = new AuthJwtParse(recordInfo);
		parser.makeDecision();
		
		DeciResult<JwtokenInfo> result = parser.getDecisionResult();
