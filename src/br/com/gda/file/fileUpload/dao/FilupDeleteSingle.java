...
public final class FilupDeleteSingle implements DaoStmt<FilupInfo> {
	private DaoStmt<FilupInfo> stmtSql;
	private DaoStmtOption<FilupInfo> stmtOption;	
	
	
	public FilupDeleteSingle(Connection conn, FilupInfo recordInfo, String schemaName) {
	...
	@Override public DaoStmt<FilupInfo> getNewInstance() {
		return new FilupDeleteSingle(stmtOption.conn, stmtOption.recordInfo, stmtOption.schemaName);
	}
	
	
	
	private class ResultParser implements DaoResultParser<FilupInfo> {
		@Override public List<FilupInfo> parseResult(ResultSet stmtResult, long lastId) throws SQLException {
			...
			FilupInfo emptyInfo = new FilupInfo();
