...
import br.com.gda.business.employeePosition.model.action.LazyEmposInsertEWT;
import br.com.gda.business.employeePosition.model.action.StdEmposSuccess;
import br.com.gda.business.employeePosition.model.checker.EmposCheckSWT;
...
final class NodeEmposInsertEWT implements DeciTree<EmposInfo> {
	private DeciTree<EmposInfo> tree;
	
	
	public NodeEmposInsertEWT(DeciTreeOption<EmposInfo> option) {
	...
	private ModelChecker<EmposInfo> buildDecisionChecker(DeciTreeOption<EmposInfo> option) {
		...
		checker = new EmposCheckSWT(checkerOption);
		queue.add(checker);	
	...
	private List<ActionStd<EmposInfo>> buildActionsOnPassed(DeciTreeOption<EmposInfo> option) {
		...
		actions.add(new LazyEmposInsertEWT(option.conn, option.schemaName).toAction(option.recordInfos));
		return actions;
	}
