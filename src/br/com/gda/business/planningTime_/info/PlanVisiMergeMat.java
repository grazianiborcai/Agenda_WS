...
final class PlanVisiMergeMat implements InfoMergerVisitorV2<PlanInfo, MatInfo> {

	@Override public PlanInfo writeRecord(MatInfo sourceOne, PlanInfo sourceTwo) {
		...
		for (PlanDataInfo eachPlanData : sourceTwo.datas) {
			if (shouldMerge(eachPlanData, sourceOne)) {		
				MatInfo copyMat = makeCopy(sourceOne);
				resultInfo.materials.add(copyMat);
				break;
			}
		}
		
		resultInfo.materials.add(sourceOne);
		
		
		for (PlanDataInfo eachPlanData : sourceTwo.datas) {
			if (shouldMerge(eachPlanData, sourceOne)) {				
				List<PlanDataInfo> mergedResults = mergeMat(eachPlanData, sourceOne);
	...
	private void checkArgument(MatInfo sourceOne, PlanInfo sourceTwo) {	
	...
	@Override public boolean shouldWrite(MatInfo sourceOne, PlanInfo sourceTwo) {
