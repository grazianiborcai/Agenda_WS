	...
	private static class ResultParser implements DaoResultParser<OrderInfo> {
		...
		@Override public List<OrderInfo> parseResult(ResultSet stmtResult, long lastId) throws SQLException {
			...
			do {
				...
				dataInfo.codUser = stmtResult.getLong(OrderemDbTableColumn.COL_COD_USER);
				dataInfo.codSnapshot = stmtResult.getLong(OrderemDbTableColumn.COL_COD_SNAPSHOT);
				dataInfo.codOrderExt = stmtResult.getString(OrderemDbTableColumn.COL_COD_ORDER_EXT);
				dataInfo.codOrderStatus = stmtResult.getString(OrderemDbTableColumn.COL_COD_ORDER_STATUS);	
				
				
				stmtResult.getLong(OrderemDbTableColumn.COL_COD_CUSTOMER);
				if (stmtResult.wasNull() == NOT_NULL)
					dataInfo.codCustomer = stmtResult.getLong(OrderemDbTableColumn.COL_COD_CUSTOMER);
				
				
				stmtResult.getString(OrderemDbTableColumn.COL_COD_LANGUAGE);
				if (stmtResult.wasNull() == NOT_NULL)
					dataInfo.codLanguage = stmtResult.getString(OrderemDbTableColumn.COL_COD_LANGUAGE);
				
				Timestamp lastChanged = stmtResult.getTimestamp(OrderemDbTableColumn.COL_LAST_CHANGED);
				if (lastChanged != null)
