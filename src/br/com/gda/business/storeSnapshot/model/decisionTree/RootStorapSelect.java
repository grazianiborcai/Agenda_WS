...
public final class RootStorapSelect extends DeciTreeReadTemplate<StorapInfo> {
	...
	@Override protected List<ActionStd<StorapInfo>> buildActionsOnPassedHook(DeciTreeOption<StorapInfo> option) {
		...
		ActionLazy<StorapInfo> mergeCurrency = new LazyStoreMergeCurrency(option.conn, option.schemaName);
		ActionLazy<StorapInfo> mergeTimezone = new LazyStoreMergeTimezone(option.conn, option.schemaName);
		ActionLazy<StorapInfo> mergePerson = new LazyStoreMergePerson(option.conn, option.schemaName);
