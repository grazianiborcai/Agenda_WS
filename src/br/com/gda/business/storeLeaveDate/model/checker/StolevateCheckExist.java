...
import br.com.gda.business.storeLeaveDate.model.action.StdStolevateEnforceKey;
...
public final class StolevateCheckExist extends ModelCheckerTemplateAction_<StolevateInfo> {
	
	public StolevateCheckExist(ModelCheckerOption option) {
		super(option);
	}
	...
	@Override protected ActionStd<StolevateInfo> buildActionHook(StolevateInfo recordInfo, Connection conn, String schemaName) {
		...
		ActionStd<StolevateInfo> enforceKey = new StdStolevateEnforceKey(option);
		ActionLazy<StolevateInfo> select = new LazyStolevateSelectKey(option.conn, option.schemaName);
