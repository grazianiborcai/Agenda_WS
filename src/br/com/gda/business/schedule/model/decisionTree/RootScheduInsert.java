...
import br.com.gda.business.schedule.model.action.LazyScheduInsert;
import br.com.gda.business.schedule.model.action.LazyScheduMergeMat;
import br.com.gda.business.schedule.model.action.LazyScheduNodeMat;
import br.com.gda.business.schedule.model.action.LazyScheduEnforceLChanged;
import br.com.gda.business.schedule.model.checker.SheduCheckEmp;
import br.com.gda.business.schedule.model.checker.SheduCheckEmpmat;
import br.com.gda.business.schedule.model.checker.SheduCheckEmpos;
import br.com.gda.business.schedule.model.checker.SheduCheckMatore;
...
public final class RootScheduInsert extends DeciTreeWriteTemplate<ScheduInfo> {
	
	public RootScheduInsert(DeciTreeOption<ScheduInfo> option) {
		super(option);
	}
	...
	@Override protected ModelChecker<ScheduInfo> buildDecisionCheckerHook(DeciTreeOption<ScheduInfo> option) {
		...
		checkerOption.schemaName = option.schemaName;
		checkerOption.expectedResult = EXIST_ON_DB;	
		checker = new SheduCheckMatore(checkerOption);
		queue.add(checker);
		...
		checkerOption.schemaName = option.schemaName;
		checkerOption.expectedResult = EXIST_ON_DB;	
		checker = new SheduCheckEmp(checkerOption);
		queue.add(checker);
		...
		checkerOption.schemaName = option.schemaName;
		checkerOption.expectedResult = EXIST_ON_DB;	
		checker = new SheduCheckEmpos(checkerOption);
		queue.add(checker);
		...
		checkerOption.schemaName = option.schemaName;
		checkerOption.expectedResult = EXIST_ON_DB;	
		checker = new SheduCheckEmpmat(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStd<ScheduInfo>> buildActionsOnPassedHook(DeciTreeOption<ScheduInfo> option) {
		...
		ActionLazy<ScheduInfo> mergeMat = new LazyScheduMergeMat(option.conn, option.schemaName);
		ActionLazy<ScheduInfo> nodeMat = new LazyScheduNodeMat(option.conn, option.schemaName);
		ActionLazy<ScheduInfo> insert = new LazyScheduInsert(option.conn, option.schemaName);
		
		nodeOrder.addPostAction(enforceLChanged);
		enforceLChanged.addPostAction(mergeMat);
		mergeMat.addPostAction(nodeMat);
		nodeMat.addPostAction(insert);
