...
public final class MatCheckExistText extends ModelCheckerTemplate<MatInfo> {
	...
	@Override protected boolean checkHook(MatInfo recordInfo, Connection conn, String schemaName) {	
		try {
			...
			for (MatInfo eachSet : resultset) {
				if (eachSet.codLanguage == null)
					return NO_ENTRY_FOUND;
			}
	...
	@Override protected String makeFailureExplanationHook(boolean checkerResult) {		
		if (makeFailureCodeHook(checkerResult) == SystemCode.MATERIAL_TEXT_ALREADY_EXIST)
			return SystemMessage.MATERIAL_TEXT_ALREALDY_EXIST;
		
		return SystemMessage.MATERIAL_TEXT_NOT_FOUND;
	}
