...
public final class StoreStmtExecDelete implements SqlStmtExec<StoreInfo> {
	private List<SqlStmt<StoreInfo>> sqlStatements;
	private SqlStmtExec<StoreInfo> helper;
	
	public StoreStmtExecDelete(List<SqlStmtExecOption<StoreInfo>> options) {
		if (options == null) 
	...
	private void prepareStatement(List<SqlStmtExecOption<StoreInfo>> options) {
		...
		for (SqlStmtExecOption<StoreInfo> eachOption : options) {
			SqlStmt<StoreInfo> sqlStatement = new StoreStmtDelete(eachOption.conn, eachOption.recordInfo, eachOption.schemaName);
