...
import br.com.gda.business.form.model.action.StdAddressFormEnforceDefault;
import br.com.gda.business.form.model.action.StdAddressFormSelect;
import br.com.gda.business.form.model.checker.AddressFormCheckExist;
...
public final class NodeAddressFormSelect implements DeciTree<AddressFormInfo> {
	private DeciTree<AddressFormInfo> tree;
	
	
	public NodeAddressFormSelect(DeciTreeOption<AddressFormInfo> option) {
	...
	private ModelChecker<AddressFormInfo> buildDecisionChecker(DeciTreeOption<AddressFormInfo> option) {
		...
		checker = new AddressFormCheckExist(checkerOption);
		queue.add(checker);
	...
	private List<ActionStd<AddressFormInfo>> buildActionsOnPassed(DeciTreeOption<AddressFormInfo> option) {
		...
		actions.add(new StdAddressFormSelect(option));
		return actions;
	}
	
	
	
	private List<ActionStd<AddressFormInfo>> buildActionsOnFailed(DeciTreeOption<AddressFormInfo> option) {
		List<ActionStd<AddressFormInfo>> actions = new ArrayList<>();
		
		actions.add(new StdAddressFormEnforceDefault(option));
		return actions;
	}
