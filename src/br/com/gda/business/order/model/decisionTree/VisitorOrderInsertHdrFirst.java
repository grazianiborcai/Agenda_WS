...
final class VisitorOrderInsertHdrFirst implements DeciActionTransVisitor<OrderInfo> {
	...
	private List<OrderInfo> execAction() {
		DeciAction<OrderInfo> action = buildAction();
	...
	private DeciAction<OrderInfo> buildAction() {
		DeciAction<OrderInfo> firstRow = new ActionOrderFirstRow(selOption);
		DeciActionHandler<OrderInfo> insertHdr = new HandlerOrderInsertHdr(selOption.conn, selOption.schemaName);
