...
final class ActionOrderMergeStore implements DeciAction<OrderInfo> {
	private DeciAction<OrderInfo> actionHelper;	
	
	
	public ActionOrderMergeStore(DeciTreeOption<OrderInfo> option) {			
		actionHelper = new DeciActionHelperTrans<>(option.recordInfos, new VisitorOrderMergeStore(option.conn, option.schemaName));
	}
