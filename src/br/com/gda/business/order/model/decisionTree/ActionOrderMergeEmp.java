...
final class ActionOrderMergeEmp implements DeciAction<OrderInfo> {
	private DeciAction<OrderInfo> actionHelper;	
	
	
	public ActionOrderMergeEmp(DeciTreeOption<OrderInfo> option) {			
		actionHelper = new DeciActionHelperTrans<>(option.recordInfos, new VisitorOrderMergeEmp(option.conn, option.schemaName));
	}
