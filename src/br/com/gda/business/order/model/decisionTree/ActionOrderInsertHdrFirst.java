...
final class ActionOrderInsertHdrFirst implements DeciAction<OrderInfo> {
	private DeciAction<OrderInfo> actionHelper;	
	
	
	public ActionOrderInsertHdrFirst(DeciTreeOption<OrderInfo> option) {			
		actionHelper = new DeciActionHelperTrans<>(option.recordInfos, new VisitorOrderInsertHdrFirst(option.conn, option.schemaName));
	}
