...
public final class EmpWtimeSelectExec implements SqlStmtExec<EmpWTimeInfo> {
	private List<SqlStmt<EmpWTimeInfo>> sqlStatements;
	private SqlStmtExec<EmpWTimeInfo> helper;
	
	public EmpWtimeSelectExec(List<SqlStmtExecOption<EmpWTimeInfo>> options) {
		if (options == null) 
	...
	private void prepareStatement(List<SqlStmtExecOption<EmpWTimeInfo>> options) {
		...
		for (SqlStmtExecOption<EmpWTimeInfo> eachOption : options) {
			SqlStmt<EmpWTimeInfo> sqlStatement = new EmpWtimeSelect(eachOption.conn, eachOption.recordInfo, eachOption.schemaName);
