...
public abstract class ActionVisitorTemplateStmt<T extends InfoRecord> implements ActionVisitor<T> {	
	...
	private DaoStmtExec<T> buildStmtExec(List<DaoStmtExecOption<T>> stmtOptions) {
		return buildStmtExecHook(stmtOptions);
	}
	
	
	
	protected DaoStmtExec<T> buildStmtExecHook(List<DaoStmtExecOption<T>> stmtOptions) {
	...
	private void closeStmtExec(DaoStmtExec<T> exec) {
		if (exec == null)
			return;
