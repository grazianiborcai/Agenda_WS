...
import br.com.mind5.webhook.pagarmeHook.model.action.PagookVisiEnforceUpperCase;
import br.com.mind5.webhook.pagarmeHook.model.action.PagookVisiMergeDaemon;
...
public final class PagookRootInsert extends DeciTreeTemplateWrite<PagookInfo> {
	...
	@Override protected List<ActionStd<PagookInfo>> buildActionsOnPassedHook(DeciTreeOption<PagookInfo> option) {
		...
		ActionLazy<PagookInfo> enforceUpperCase    = new ActionLazyCommom<PagookInfo>(option, PagookVisiEnforceUpperCase.class);		
		...
		ActionLazy<PagookInfo> mergeDaemon         = new ActionLazyCommom<PagookInfo>(option, PagookVisiMergeDaemon.class);
		ActionLazy<PagookInfo> nodeL1              = new ActionLazyCommom<PagookInfo>(option, PagookVisiNodeInsertL1.class);
		
		enforceIdPayment.addPostAction(enforceUpperCase);
		enforceUpperCase.addPostAction(enforceCreatedOn);
		enforceCreatedOn.addPostAction(enforceLastChanged);
		enforceLastChanged.addPostAction(mergeDaemon);
		mergeDaemon.addPostAction(nodeL1);
