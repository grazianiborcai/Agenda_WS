...
public final class WokefumoipDaoSelectSingle extends DaoStmtTemplate<WokefumoipInfo> {
	private final String MAIN_TABLE = DaoDbTable.PAY_ORDER_ITM_TABLE;
	
	
	public WokefumoipDaoSelectSingle(Connection conn, WokefumoipInfo recordInfo, String schemaName) {
	...
	@Override protected String buildWhereClauseHook(String tableName, WokefumoipInfo recordInfo) {
		...
		DaoStmtWhere whereClause = new WokefumoipDaoWhere(whereOption, tableName, recordInfo);
		...
		return new DaoResultParser<WokefumoipInfo>() {
			@Override public List<WokefumoipInfo> parseResult(WokefumoipInfo redcordInfo, ResultSet stmtResult, long lastId) throws SQLException {
				...
				do {
					...
					dataInfo.codOwner = stmtResult.getLong(WokefumoipDaoDbTableColumn.COL_COD_OWNER);
					dataInfo.codPayOrder = stmtResult.getLong(WokefumoipDaoDbTableColumn.COL_COD_PAY_ORDER);	
					dataInfo.idPaymentPartner = stmtResult.getString(WokefumoipDaoDbTableColumn.COL_ID_PAYMENT_PARTNER);				
