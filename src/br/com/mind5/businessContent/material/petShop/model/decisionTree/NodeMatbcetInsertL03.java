...
public final class NodeMatbcetInsertL03 extends DeciTreeTemplateWriteV2<MatbcetInfo> {
	...
	@Override protected List<ActionStdV1<MatbcetInfo>> buildActionsOnPassedHook(DeciTreeOption<MatbcetInfo> option) {
		...
		ActionStdV1<MatbcetInfo> enforceDogHaircut = new StdMatbcetEnforceDogHaircut(option);	
		ActionLazyV1<MatbcetInfo> insertMat = new LazyMatbcetMatInsert(option.conn, option.schemaName);	
		ActionLazyV1<MatbcetInfo> nodeL04 = new LazyMatbcetNodeInsertL04(option.conn, option.schemaName);	
