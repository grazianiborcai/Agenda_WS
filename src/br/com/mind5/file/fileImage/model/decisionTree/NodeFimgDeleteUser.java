...
import br.com.mind5.file.fileImage.model.checker.FimgCheckFimarchUser;
...
public final class NodeFimgDeleteUser extends DeciTreeTemplateWriteV2<FimgInfo> {
	
	public NodeFimgDeleteUser(DeciTreeOption<FimgInfo> option) {
		super(option);
	}
	...
	@Override protected ModelCheckerV1<FimgInfo> buildCheckerHook(DeciTreeOption<FimgInfo> option) {
		...
		checker = new FimgCheckFimarchUser(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStdV1<FimgInfo>> buildActionsOnPassedHook(DeciTreeOption<FimgInfo> option) {
		...
		ActionStdV1<FimgInfo> delete = new RootFimgDelete(option).toAction();
