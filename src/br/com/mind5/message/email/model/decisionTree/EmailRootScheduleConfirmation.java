...
import br.com.mind5.message.email.model.action.EmailVisiEnforceScheduleConfirmation;
...
public final class EmailRootScheduleConfirmation extends DeciTreeTemplateWrite<EmailInfo> {
	
	public EmailRootScheduleConfirmation(DeciTreeOption<EmailInfo> option) {
		super(option);
	}
	...
	@Override protected List<ActionStd<EmailInfo>> buildActionsOnPassedHook(DeciTreeOption<EmailInfo> option) {
		...
		ActionLazy<EmailInfo> enforceScheduleConfirmation = new ActionLazyCommom<EmailInfo>(option, EmailVisiEnforceScheduleConfirmation.class);
