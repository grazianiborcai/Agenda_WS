...
public final class PeresmoipDaoSelectSingle extends DaoStmtTemplate<PeresmoipInfo> {
	private final String MAIN_TABLE = DaoDbTable.MOIP_PERMISSION_RESPONSE_TABLE;
	
	
	public PeresmoipDaoSelectSingle(Connection conn, PeresmoipInfo recordInfo, String schemaName) {
		...
		return new DaoResultParser<PeresmoipInfo>() {
			@Override public List<PeresmoipInfo> parseResult(PeresmoipInfo recordInfo, ResultSet stmtResult, long lastId) throws SQLException {
				...
				do {
					...
					dataInfo.codOwner = stmtResult.getLong(PeresmoipDaoDbTableColumn.COL_COD_OWNER);
					dataInfo.codStore = stmtResult.getLong(PeresmoipDaoDbTableColumn.COL_COD_STORE);
					dataInfo.isExpected = stmtResult.getBoolean(PeresmoipDaoDbTableColumn.COL_IS_EXPECTED);
					dataInfo.username = stmtResult.getString(PeresmoipDaoDbTableColumn.COL_USERNAME);
					dataInfo.lastChanged = DaoFormatter.sqlToLocalDateTime(stmtResult, PeresmoipDaoDbTableColumn.COL_LAST_CHANGED);
