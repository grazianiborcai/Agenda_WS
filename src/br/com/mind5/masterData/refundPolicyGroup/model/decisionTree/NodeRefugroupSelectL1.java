...
public final class NodeRefugroupSelectL1 extends DeciTreeTemplateReadV2<RefugroupInfo> {
	
	public NodeRefugroupSelectL1(DeciTreeOption<RefugroupInfo> option) {
		super(option);
	}
	...
	@Override protected List<ActionStdV1<RefugroupInfo>> buildActionsOnPassedHook(DeciTreeOption<RefugroupInfo> option) {
		...
		ActionStdV1<RefugroupInfo> nodelL2 = new NodeRefugroupSelectL2(option).toAction();
	...
	@Override protected List<ActionStdV1<RefugroupInfo>> buildActionsOnFailedHook(DeciTreeOption<RefugroupInfo> option) {
		List<ActionStdV1<RefugroupInfo>> actions = new ArrayList<>();
		
		ActionStdV1<RefugroupInfo> nodelDefault = new RootRefugroupDefault(option).toAction();
		
		actions.add(nodelDefault);
		return actions;
	}
}
