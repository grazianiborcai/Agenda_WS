...
import br.com.mind5.masterData.materialGroupOwner.model.action.MatoupowVisiEnforceLockedOn;
...
public final class MatoupowNodeInactivate extends DeciTreeTemplateWrite<MatoupowInfo> {
	
	public MatoupowNodeInactivate(DeciTreeOption<MatoupowInfo> option) {
		super(option);
	}
	...
	@Override protected List<ActionStd<MatoupowInfo>> buildActionsOnPassedHook(DeciTreeOption<MatoupowInfo> option) {
		...
		ActionLazy<MatoupowInfo> enforceLockedOn = new ActionLazyCommom<MatoupowInfo>(option, MatoupowVisiEnforceLockedOn.class);
	...
	@Override protected List<ActionStd<MatoupowInfo>> buildActionsOnFailedHook(DeciTreeOption<MatoupowInfo> option) {
		...
		insert.addPostAction(enforceLockedOn);
		enforceLockedOn.addPostAction(nodeUpdate);
		nodeUpdate.addPostAction(select);
