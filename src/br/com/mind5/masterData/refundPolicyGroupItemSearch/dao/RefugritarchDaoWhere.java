...
final class RefugritarchDaoWhere implements DaoStmtWhere {
	private String whereClause;	
	
	
	public RefugritarchDaoWhere(DaoWhereBuilderOption whereOption, String tableName, RefugritarchInfo recordInfo) {
	...
	private void generateWhereClause(DaoWhereBuilderOption whereOption, String tableName, RefugritarchInfo recordInfo) {
		...
		for (DaoColumn eachColumn : columns) {
			switch(eachColumn.columnName) {			
				case RefugritarchDaoDbTableColumn.COL_COD_REFUND_POLICY_GROUP :
					builder.addClauseEqualAnd(eachColumn, DaoFormatter.numberToString(recordInfo.codRefundPolicyGroup));
					break;
				
				case RefugritarchDaoDbTableColumn.COL_COD_REFUND_POLICY :
