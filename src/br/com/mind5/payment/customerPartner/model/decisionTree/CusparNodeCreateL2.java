...
import br.com.mind5.payment.customerPartner.model.checker.CusparCheckCreate;
import br.com.mind5.payment.customerPartner.model.checker.CusparCheckCus;
...
public final class CusparNodeCreateL2 extends DeciTreeTemplateWrite<CusparInfo> {
	...
	@Override protected ModelChecker<CusparInfo> buildCheckerHook(DeciTreeOption<CusparInfo> option) {		
		...
		checker = new CusparCheckCreate(checkerOption);
		queue.add(checker);
		...
		checkerOption.schemaName = option.schemaName;
		checkerOption.expectedResult = ModelCheckerOption.EXIST_ON_DB;	
		checker = new CusparCheckCus(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStd<CusparInfo>> buildActionsOnPassedHook(DeciTreeOption<CusparInfo> option) {
		...
		ActionStd<CusparInfo> nodeL3 = new CusparNodeCreateL3(option).toAction();	
