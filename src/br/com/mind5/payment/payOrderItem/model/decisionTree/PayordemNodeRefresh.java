...
import br.com.mind5.payment.payOrderItem.model.action.PayordemVisiOrderemRefresh;
import br.com.mind5.payment.payOrderItem.model.checker.PayordemCheckHasOrderem;

public final class PayordemNodeRefresh extends DeciTreeTemplateWrite<PayordemInfo> {
	
	public PayordemNodeRefresh(DeciTreeOption<PayordemInfo> option) {
		super(option);
	}
	...
	@Override protected List<ActionStd<PayordemInfo>> buildActionsOnPassedHook(DeciTreeOption<PayordemInfo> option) {
		...
		ActionStd<PayordemInfo> orderemRefresh = new ActionStdCommom<PayordemInfo>(option, PayordemVisiOrderemRefresh.class);
		
		actions.add(orderemRefresh);
		return actions;
	}
