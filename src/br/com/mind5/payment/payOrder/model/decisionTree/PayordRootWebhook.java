...
import br.com.mind5.payment.payOrder.model.action.PayordVisiEnforceHasWebhookEventOn;
...
public final class PayordRootWebhook extends DeciTreeTemplateWrite<PayordInfo> {
	
	public PayordRootWebhook(DeciTreeOption<PayordInfo> option) {
		super(option);
	}
	...
	@Override protected List<ActionStd<PayordInfo>> buildActionsOnPassedHook(DeciTreeOption<PayordInfo> option) {
		...
		ActionStd <PayordInfo> select       			= new ActionStdCommom <PayordInfo>(option, PayordVisiMergeToSelect.class);
		ActionLazy<PayordInfo> enforceLChanged  		= new ActionLazyCommom<PayordInfo>(option, PayordVisiEnforceLChanged.class);
		ActionLazy<PayordInfo> enforceHasWebhookEventOn = new ActionLazyCommom<PayordInfo>(option, PayordVisiEnforceHasWebhookEventOn.class);
		ActionLazy<PayordInfo> updatePayord 			= new ActionLazyCommom<PayordInfo>(option, PayordVisiDaoUpdate.class);
		
		select.addPostAction(enforceLChanged);
		enforceLChanged.addPostAction(enforceHasWebhookEventOn);
		enforceHasWebhookEventOn.addPostAction(updatePayord);
