...
public final class CusparchDbTableColumn extends DaoDbTableColumnTemplate {
	...
	public CusparchDbTableColumn() {
		super(CusparchDbTableColumn.class);
	}
	...
	private void buildPayCustomerTable() {
		...
		oneColumn.columnName = COL_COD_PAYCUS;
		...
		oneColumn = new DaoColumn();
		oneColumn.tableName = TABLE_NAME;
		oneColumn.columnName = COL_COMPOUND_ID;
		oneColumn.isPK = NEGATIVE;
		oneColumn.isLookUp = NEGATIVE;
		oneColumn.isAutoIncremented = NEGATIVE;
		columns.add(oneColumn);	
		...
		oneColumn = new DaoColumn();
		oneColumn.tableName = TABLE_NAME;
		oneColumn.columnName = COL_CUSTOMER_ID;
		oneColumn.isPK = NEGATIVE;
		oneColumn.isLookUp = NEGATIVE;
		oneColumn.isAutoIncremented = NEGATIVE;
		columns.add(oneColumn);	
		
		tableColumns.put(DaoDbTable.PAY_CUS_SEARCH_VIEW, columns);
	}
}
