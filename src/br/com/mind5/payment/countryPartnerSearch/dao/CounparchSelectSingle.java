...
public final class CounparchSelectSingle extends DaoStmtTemplate<CounparchInfo> {
	private final String MAIN_TABLE = DaoDbTable.PAY_PARTNER_COUNTRY_TABLE;
	
	
	public CounparchSelectSingle(Connection conn, CounparchInfo recordInfo, String schemaName) {
	...
	@Override protected String buildWhereClauseHook(String tableName, CounparchInfo recordInfo) {
		...
		DaoStmtWhere whereClause = new CounparchWhere(whereOption, tableName, recordInfo);
		...
		return new DaoResultParser<CounparchInfo>() {
			@Override public List<CounparchInfo> parseResult(CounparchInfo recordInfo, ResultSet stmtResult, long lastId) throws SQLException {
				...
				do {
					...
					dataInfo.codCountry = stmtResult.getString(CounparchDbTableColumn.COL_COD_COUNTRY);
					dataInfo.codPayPartner = stmtResult.getInt(CounparchDbTableColumn.COL_COD_PAY_PARTNER);
					dataInfo.isDefault = DaoFormatter.sqlToBoole(stmtResult, CounparchDbTableColumn.COL_IS_DEFAULT);					
