...
public final class SysparchSelectSingle extends DaoStmtTemplate<SysparchInfo> {
	private final String MAIN_TABLE = DaoDbTable.SYS_PAY_PARTNER_TABLE;
	
	
	public SysparchSelectSingle(Connection conn, SysparchInfo recordInfo, String schemaName) {
	...
	@Override protected String buildWhereClauseHook(String tableName, SysparchInfo recordInfo) {
		...
		DaoStmtWhere whereClause = new SysparchWhere(whereOption, tableName, recordInfo);
		...
		return new DaoResultParser<SysparchInfo>() {
			@Override public List<SysparchInfo> parseResult(SysparchInfo recordInfo, ResultSet stmtResult, long lastId) throws SQLException {
				...
				do {
					...
					dataInfo.idPayPartnerSystem = stmtResult.getString(SysparchDbTableColumn.COL_ID_PAY_PARTNER_SYSTEM);
					dataInfo.idPayPartnerApp = stmtResult.getString(SysparchDbTableColumn.COL_ID_PAY_PARTNER_APP);
					dataInfo.codPayPartner = stmtResult.getInt(SysparchDbTableColumn.COL_COD_PAY_PARTNER);		
