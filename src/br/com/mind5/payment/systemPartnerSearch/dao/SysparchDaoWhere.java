...
final class SysparchDaoWhere implements DaoStmtWhere {
	private String whereClause;	
	
	
	public SysparchDaoWhere(DaoWhereBuilderOption whereOption, String tableName, SysparchInfo recordInfo) {
	...
	private void generateWhereClause(DaoWhereBuilderOption whereOption, String tableName, SysparchInfo recordInfo) {
		...
		for (DaoColumn eachColumn : columns) {			
			switch(eachColumn.columnName) {
				case SysparchDaoDbTableColumn.COL_COD_PAY_PARTNER:
					builder.addClauseEqualAnd(eachColumn, DaoFormatter.numberToString(recordInfo.codPayPartner));
					break;
					
				case SysparchDaoDbTableColumn.COL_ID_PAY_PARTNER_SYSTEM:
