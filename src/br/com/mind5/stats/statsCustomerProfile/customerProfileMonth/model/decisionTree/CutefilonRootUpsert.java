...
import br.com.mind5.stats.statsCustomerProfile.customerProfileMonth.model.checker.CutefilonCheckWrite;


public final class CutefilonRootUpsert extends DeciTreeTemplateWrite<CutefilonInfo> {
	
	public CutefilonRootUpsert(DeciTreeOption<CutefilonInfo> option) {
		super(option);
	}
	...
	@Override protected ModelChecker<CutefilonInfo> buildCheckerHook(DeciTreeOption<CutefilonInfo> option) {
		...
		checker = new CutefilonCheckWrite(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStd<CutefilonInfo>> buildActionsOnPassedHook(DeciTreeOption<CutefilonInfo> option) {
		...
		ActionStd<CutefilonInfo> nodeL1 = new CutefilonNodeUpsert(option).toAction();		
