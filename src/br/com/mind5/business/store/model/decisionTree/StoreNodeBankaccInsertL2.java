...
import br.com.mind5.business.store.model.checker.StoreCheckBankaccDoc;
...
public final class StoreNodeBankaccInsertL2 extends DeciTreeTemplateWrite<StoreInfo> {
	
	public StoreNodeBankaccInsertL2(DeciTreeOption<StoreInfo> option) {
		super(option);
	}
	...
	@Override protected ModelChecker<StoreInfo> buildCheckerHook(DeciTreeOption<StoreInfo> option) {
		...
		checker = new StoreCheckHasBankacc(checkerOption);
		queue.add(checker);
		...
		checkerOption.schemaName = option.schemaName;
		checkerOption.expectedResult = ModelCheckerOption.SUCCESS;		
		checker = new StoreCheckBankaccDoc(checkerOption);
		queue.add(checker);
