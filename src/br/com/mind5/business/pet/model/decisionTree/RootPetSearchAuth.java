...
import br.com.mind5.business.pet.model.action.LazyPetRootSearch;
...
public final class RootPetSearchAuth extends DeciTreeTemplateWrite<PetInfo> {
	
	public RootPetSearchAuth(DeciTreeOption<PetInfo> option) {
		super(option);
	}
	...
	@Override protected List<ActionStd<PetInfo>> buildActionsOnPassedHook(DeciTreeOption<PetInfo> option) {
		...
		ActionStd<PetInfo> nodeAuth = new NodePetSearchAuthL1(option).toAction();
		ActionLazy<PetInfo> search = new LazyPetRootSearch(option.conn, option.schemaName);
