...
import br.com.mind5.business.notes.model.checker.NotesCheckSafeNote;
...
public final class NotesNodeSafe extends DeciTreeTemplateWrite<NotesInfo> {
	
	public NotesNodeSafe(DeciTreeOption<NotesInfo> option) {
		super(option);
	}
	...
	@Override protected ModelChecker<NotesInfo> buildCheckerHook(DeciTreeOption<NotesInfo> option) {
		...
		checker = new NotesCheckSafeNote(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStd<NotesInfo>> buildActionsOnPassedHook(DeciTreeOption<NotesInfo> option) {
		...
		ActionStd<NotesInfo> success = new ActionStdSuccessCommom<NotesInfo>(option);	
