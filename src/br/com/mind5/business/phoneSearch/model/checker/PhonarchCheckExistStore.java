...
public final class PhonarchCheckExistStore extends ModelCheckerTemplateAction<PhonarchInfo, PhonarchInfo> {
	
	public PhonarchCheckExistStore(ModelCheckerOption option) {
	...
	@Override protected ActionStd<PhonarchInfo> buildActionHook(DeciTreeOption<PhonarchInfo> option) {
		ActionStd<PhonarchInfo> select = new PhonarchRootSelectStore(option).toAction();
		return select;
	}
