...
public final class AreaPhoneSelectSingle implements DaoStmt<AreaPhoneInfo> {
	private final String LT_ATTR = DaoDbTable.AREA_PHONE_TABLE;
	private final String RT_TEXT = DaoDbTable.AREA_PHONE_TEXT_TABLE;
	
	private DaoStmt<AreaPhoneInfo> stmtSql;
	private DaoStmtOption_<AreaPhoneInfo> stmtOption;
	...
	private void buildStmtOption(Connection conn, AreaPhoneInfo recordInfo, String schemaName) {
	...
	private String buildWhereClause() {
		...
		DaoStmtWhere whereClause = new AreaPhoneWhere(whereOption, stmtOption.tableName, stmtOption.recordInfo);
	...
	private DaoJoin buildJoinUnitText() {
		...
		oneColumn.leftColumnName = MasterDataDbTableColumn.COL_COD_COUNTRY_PHONE;
		oneColumn.rightColumnName = MasterDataDbTableColumn.COL_COD_COUNTRY_PHONE;
		joinColumns.add(oneColumn);
		
		
		oneColumn = new DaoJoinColumn();
		oneColumn.leftTableName = LT_ATTR;
		oneColumn.leftColumnName = MasterDataDbTableColumn.COL_COD_AREA_PHONE;
		oneColumn.rightColumnName = MasterDataDbTableColumn.COL_COD_AREA_PHONE;
		joinColumns.add(oneColumn);
	...
	@Override public DaoStmt<AreaPhoneInfo> getNewInstance() {
		return new AreaPhoneSelectSingle(stmtOption.conn, stmtOption.recordInfo, stmtOption.schemaName);
	}
	
	
	
	private class ResultParser implements DaoResultParser_<AreaPhoneInfo> {
