...
public final class BankaccCheckExistStore extends ModelCheckerTemplateAction<BankaccInfo, BankaccInfo> {	
	
	public BankaccCheckExistStore(ModelCheckerOption option) {
	...
	@Override protected ActionStd<BankaccInfo> buildActionHook(DeciTreeOption<BankaccInfo> option) {
		ActionStd<BankaccInfo> select = new BankaccRootSearchStore(option).toAction();
		return select;
	}
