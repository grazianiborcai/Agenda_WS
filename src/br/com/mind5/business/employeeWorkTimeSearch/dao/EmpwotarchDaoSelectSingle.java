...
public final class EmpwotarchDaoSelectSingle extends DaoStmtTemplate<EmpwotarchInfo> {
	private final String MAIN_TABLE = DaoDbTable.EMP_WT_TABLE;	
	

	public EmpwotarchDaoSelectSingle(Connection conn, EmpwotarchInfo recordInfo, String schemaName) {
	...
	@Override protected String buildWhereClauseHook(String tableName, EmpwotarchInfo recordInfo) {
		...
		DaoStmtWhere whereClause = new EmpwotarchDaoWhere(whereOption, tableName, recordInfo);
		...
		return new DaoResultParser<EmpwotarchInfo>() {
			@Override public List<EmpwotarchInfo> parseResult(EmpwotarchInfo redcordInfo, ResultSet stmtResult, long lastId) throws SQLException {
				...
				do {				
					...
					dataInfo.codOwner = stmtResult.getLong(EmpwotarchDaoDbTableColumn.COL_COD_OWNER);
					dataInfo.codStore = stmtResult.getLong(EmpwotarchDaoDbTableColumn.COL_COD_STORE);
					dataInfo.codEmployee = stmtResult.getLong(EmpwotarchDaoDbTableColumn.COL_COD_EMPLOYEE);
					dataInfo.codWeekday = stmtResult.getInt(EmpwotarchDaoDbTableColumn.COL_COD_WEEKDAY);
					dataInfo.beginTime = DaoFormatter.sqlToLocalTime(stmtResult, EmpwotarchDaoDbTableColumn.COL_BEGIN_TIME);
					dataInfo.endTime = DaoFormatter.sqlToLocalTime(stmtResult, EmpwotarchDaoDbTableColumn.COL_END_TIME);
					dataInfo.recordMode = stmtResult.getString(EmpwotarchDaoDbTableColumn.COL_RECORD_MODE);			
