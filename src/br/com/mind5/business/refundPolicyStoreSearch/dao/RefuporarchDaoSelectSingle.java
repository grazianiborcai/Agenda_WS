...
public final class RefuporarchDaoSelectSingle extends DaoStmtTemplate<RefuporarchInfo> {
	private final String MAIN_TABLE = DaoDbTable.REFUND_POLICY_STORE_TABLE;
	
	
	public RefuporarchDaoSelectSingle(Connection conn, RefuporarchInfo recordInfo, String schemaName) {
	...
	@Override protected String buildWhereClauseHook(String tableName, RefuporarchInfo recordInfo) {
		...
		DaoStmtWhere whereClause = new RefuporarchDaoWhere(whereOption, tableName, recordInfo);
		...
		return new DaoResultParser<RefuporarchInfo>() {
			@Override public List<RefuporarchInfo> parseResult(RefuporarchInfo recordInfo, ResultSet stmtResult, long lastId) throws SQLException {
				...
				do {				
					...
					dataInfo.codStore = DaoFormatter.sqlToLong(stmtResult, RefuporarchDaoDbTableColumn.COL_COD_STORE);
					dataInfo.codOwner = DaoFormatter.sqlToLong(stmtResult, RefuporarchDaoDbTableColumn.COL_COD_OWNER);
					dataInfo.codRefundPolicyGroup = DaoFormatter.sqlToInt(stmtResult, RefuporarchDaoDbTableColumn.COL_COD_REFUND_POLICY_GROUP);					
					dataInfo.recordMode = stmtResult.getString(RefuporarchDaoDbTableColumn.COL_RECORD_MODE);
