...
import br.com.mind5.business.addressSearch.model.action.AddarchVisiEnforceUserAddressKey;
...
import br.com.mind5.business.addressSearch.model.checker.AddarchCheckReadUserAddress;
...
public final class AddarchRootSelectUserAddress extends DeciTreeTemplateWrite<AddarchInfo> {
	
	public AddarchRootSelectUserAddress(DeciTreeOption<AddarchInfo> option) {
		super(option);
	}
	...
	@Override protected ModelChecker<AddarchInfo> buildCheckerHook(DeciTreeOption<AddarchInfo> option) {
		...
		checker = new AddarchCheckReadUserAddress(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStd<AddarchInfo>> buildActionsOnPassedHook(DeciTreeOption<AddarchInfo> option) {
		...
		ActionStd<AddarchInfo>  enforceUserAddressKey = new ActionStdCommom<AddarchInfo> (option, AddarchVisiEnforceUserAddressKey.class);
		ActionLazy<AddarchInfo> select                = new ActionLazyCommom<AddarchInfo>(option, AddarchVisiRootSelect.class);
		
		enforceUserAddressKey.addPostAction(select);
		
		actions.add(enforceUserAddressKey);
		return actions;
	}
}
