...
public final class NodeMatoreUpsertL1 extends DeciTreeTemplateWriteV2<MatoreInfo> {
	
	public NodeMatoreUpsertL1(DeciTreeOption<MatoreInfo> option) {
		super(option);
	}
	...
	@Override protected ModelCheckerV1<MatoreInfo> buildCheckerHook(DeciTreeOption<MatoreInfo> option) {
		...
		checker = new MatoreCheckHasMatCateg(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStdV2<MatoreInfo>> buildActionsOnPassedHook(DeciTreeOption<MatoreInfo> option) {
		...
		ActionStdV2<MatoreInfo> nodeL2 = new NodeMatoreUpsertL2(option).toAction();		
		actions.add(nodeL2);
