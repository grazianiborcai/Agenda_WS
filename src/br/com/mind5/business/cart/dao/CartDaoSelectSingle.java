...
public final class CartDaoSelectSingle extends DaoStmtTemplate<CartInfo> {
	private final String MAIN_TABLE = DaoDbTable.CART_HDR_TABLE;
	
	
	public CartDaoSelectSingle(Connection conn, CartInfo recordInfo, String schemaName) {
		...
		return new DaoResultParser<CartInfo>() {
			@Override public List<CartInfo> parseResult(CartInfo recordInfo, ResultSet stmtResult, long lastId) throws SQLException {				
				...
				do {
					...
					dataInfo.codOwner = stmtResult.getLong(CartDaoDbTableColumn.COL_COD_OWNER);	
					dataInfo.codUser = stmtResult.getLong(CartDaoDbTableColumn.COL_COD_USER);	
					dataInfo.lastChanged = DaoFormatter.sqlToLocalDateTime(stmtResult, CartDaoDbTableColumn.COL_LAST_CHANGED);	
					dataInfo.expiryOn = DaoFormatter.sqlToLocalDateTime(stmtResult, CartDaoDbTableColumn.COL_EXPIRY_ON);
