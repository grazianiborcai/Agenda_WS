...
import br.com.mind5.business.orderItem.model.action.LazyOrderemNodeRefresh;
...
import br.com.mind5.business.orderItem.model.checker.OrderemCheckRefresh;
...
public final class RootOrderemRefresh extends DeciTreeTemplateWriteV2<OrderemInfo> {
	
	public RootOrderemRefresh(DeciTreeOption<OrderemInfo> option) {
		super(option);
	}
	...
	@Override protected ModelCheckerV1<OrderemInfo> buildCheckerHook(DeciTreeOption<OrderemInfo> option) {
		...
		checker = new OrderemCheckRefresh(checkerOption);
		queue.add(checker);
	...
	@Override protected List<ActionStdV2<OrderemInfo>> buildActionsOnPassedHook(DeciTreeOption<OrderemInfo> option) {
		...
		ActionStdV2<OrderemInfo> mergeToUpdate = new StdOrderemMergeToUpdate(option);
		ActionLazy<OrderemInfo> nodeRefresh = new LazyOrderemNodeRefresh(option.conn, option.schemaName);
