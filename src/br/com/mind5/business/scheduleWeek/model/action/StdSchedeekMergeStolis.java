...
final class StdSchedeekMergeStolis implements ActionStdV1<SchedeekInfo> {
	private ActionStdV1<SchedeekInfo> actionHelper;	
	
	
	public StdSchedeekMergeStolis(DeciTreeOption<SchedeekInfo> option) {			
		actionHelper = new ActionStdHelperMerge<>(option.recordInfos, new VisiSchedeekMergeStolis(option.conn, option.schemaName));
	}
